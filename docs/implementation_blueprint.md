...
**§6 completion (migration executed, suite green under the pinned stack).** Final lock: `sae-lens` 6.44.2, `transformers` **5.12.1** (pinned to the training-time cluster manifest — matches what produced the checkpoints, crossing the 4→5 major deliberately, not the solver's lower 4.57), `datasets` 5.0.0, `transformer-lens` 3.2.1, `huggingface-hub` 1.24.0, `tokenizers` 0.22.2; `torch` 2.13.0 / `accelerate` 0.33 held. **`numpy` stayed 1.26.4 — the ED-19 numpy-2 conflict did NOT dissolve; SS8 `lodestar-eval` stays paused** (the earlier "may dissolve" upside did not materialize). Two model-loading-surface behavior changes were found and handled, both confirmed identical on Windows and Linux (version behavior, not platform): (1) `transformer-lens` 3.2.1's `HookedTransformer(cfg, tokenizer=…)` **silently drops the trailing `<eos>`** — the delta-golden prompt went 33→32 tokens; (2) the KV-cache class moved (`HookedTransformerKeyValueCache` → `cache.key_value_cache.TransformerLensKeyValueCache`). `tiny_model` weights regenerated **byte-identical** (only transformers-5 save-format JSON changed); `tiny_sae` + `delta_golden` regenerated fresh; `MAX_ULP` 32→128 (measured clamp 32 / add_direction 4 under 6.x TopK, ED-26's 4× convention). **Residual gap for the §6 cluster gate:** the `<eos>`-drop behavior is on a model-loading surface whose training match must be confirmed — the full-weight forward MUST additionally confirm real-Qwen tokenization/positions are as expected, not merely that activations are finite. **Update (synced A5 now records it):** training-time `transformer-lens` was **3.4.0**, not `null` as ED-33 assumed; the lock is on **3.2.1** (same major, minor gap). Per ED-33's own prefer-training-time principle, before the first certificate either bump `transformer-lens` 3.2.1→3.4.0 (and regen fixtures/goldens if numerics move) **or** prove 3.2.1 ≡ 3.4.0 on `hook_resid_post` activations — added to this gate's checklist, since certification metrics are computed on exactly those activations. `fp32_copy` fix (`type(sae)(cfg)`) landed on all **three** sanctioned §1 duplicates (certification/metrics, interventions/hooks, characterization/indexer — the third surfaced by the suite). Certification lane remains closed until the gate runs.

**ED-34 (cert-lane cluster execution — `tamia:`/`hf:` resolution, decided at first cert-lane run).** The certification lane's local-only URI handling is an **implementation defect against already-decided architecture**, not a new question: `tamia:` is a declared §3.2 scheme with no resolver; the cert-lane config schemas already accept `^(tamia|local|hf|wandb):`; ED-7 already ruled production certify runs on the cluster (GPU). One local-only gate was written once and copy-pasted across five sites (`certify`/`characterize`/`validate`/`steer` + `characterization/feature_index`). Three fixes, of which two carry rulings:
1. **`tamia:` weights (mechanical).** Add `core.uris.resolve_tamia` (reads `$SCRATCH`, joins `interplab/<value>`, raises clearly if unset — matching §3.2's documented convention); widen the five duplicated guards from `!= "local"` to `not in ("local","tamia")`. Sanctioned §1 duplication kept.
2. **`hf:` model refs — fidelity ruling, NOT a new parallel loader (overrides the engineer's from_pretrained recommendation).** `model_location` is `hf:<repo>@<rev>` (ED-29). The existing `load_local_hooked_transformer` deliberately uses the manual convert path with `fold_ln=False` / `center_writing_weights=False` / `center_unembed=False`; `HookedTransformer.from_pretrained` applies *different* defaults (`fold_ln=True`, …) → different `hook_resid_post` activations → wrong certification metrics. So `hf:` MUST be an **acquisition** scheme, not a second construction path: snapshot-download the repo pinned to `@rev` into `$SCRATCH/hf_cache` → local dir → feed the **single existing loader**. Symmetric with `tamia:` (both resolve to a local dir consumed by one loader); the new code is a pinned-download helper, never a new `HookedTransformer` build. Rationale: two model-loading paths with divergent processing flags is exactly the silent-fidelity hole ED-27/ED-32 reject — the loading path is part of *how the bytes become a function*.
3. **`tamia:`/`local:` corpus in `eval_slice` — (a) duplicate, because (b) is architecturally impossible.** `eval_slice.load_corpus_docs` must read the real training corpus (a local HF Arrow dataset cache dir, per A5 `dataset_path`). The natural reuse (`corpus.replay.open_stream`) is forbidden by Ground Rule 2 (certification → core, registry only). Promoting the primitive to `core` is **blocked**: `replay.iter_local_hf_dataset` depends on the `datasets` library and core's dependency floor is stdlib/numpy/pydantic/jsonschema only. The remaining shared-home options (widen Ground Rule 2's edges; a new datasets-capable trunk) have blast radius far exceeding a ~5-line dispatch. Ruling: **duplicate** the file-vs-directory dispatch (JSONL file vs local HF dataset cache) into `eval_slice`, mirroring `replay.open_stream`, cross-referenced in both as sanctioned twins. Deeper defect this exposes: `eval_slice`'s `local:` branch already handled JSONL *only*, diverging from `replay`'s file-vs-dir dispatch — so the fix covers local-HF-dir under **both** `local:` and `tamia:`, not merely the new scheme.
No open architectural questions remain after these three; all are defects or fidelity-preserving extensions of decided architecture. (Operational note, non-architectural: on compute nodes the pinned `hf:` download uses the warm `$SCRATCH/hf_cache` — `local_files_only` when the cache is populated on a login node first, per Alliance's usual no-internet-on-compute pattern.)
**Completeness (two more same-class sites, folded in after implementation surfaced them — flagged, not self-selected).** A defect fix is not finished while identical instances of the same bug remain in sibling code that would fail the same way on the cluster: (4) fix 1 (Gap A) applies to a **sixth** guard — `characterization/feature_index._resolve_index_dir` gates the A7 index-directory location `!= "local"`; widen to `local`/`tamia`, but only on the registry-ref-location branch — the §5.SS5 *degraded raw-local-dir* mode (a bare directory `PathOrHash`, no URI) MUST be preserved, not routed through URI resolution. (5) fix 3 (Gap C) applies to a **second** site — `jobs/characterize._load_docs_jsonl` has the identical JSONL-only reading defect `eval_slice` had (same subsystem-boundary reason: characterization cannot import `corpus`); duplicate the file-vs-dir dispatch there too, under both `local:` and `tamia:`. Both are mechanical applications of rulings already made, authorized for consistency so `characterize` does not reproduce the `certify` cluster failure.

**ED-36 (cluster offline acquisition clarification, decided at A-004) — amends ED-1; ED-33 unchanged.**
1. The cluster profile MAY supplement the Alliance wheelhouse with a transferred offline package bundle. Such a bundle is a derived acquisition cache, not vendoring and not a third environment profile, iff it remains outside the repository, is derived from the committed `pyproject.toml`/`uv.lock`, and cannot introduce or select another runtime version. Direct network installation on the cluster is forbidden; failure of the offline inputs is a stop condition, never permission to contact an index.
2. A researcher-required exact runtime version MUST be declared directly in `pyproject.toml`; therefore `transformer-lens==3.2.1` is a direct dependency. The cluster requirements file MUST be a frozen, hash-bearing export of the committed lock. CUDA torch remains the ED-1 profile exception, but its public version must equal the locked version and its exact Alliance artifact must be hashed.
3. Before any target virtualenv is created or altered, a machine-readable acquisition manifest MUST verify the hashes of `pyproject.toml`, `uv.lock`, the requirements export, and every selected file; the Linux architecture, CPython version and ABI; generator/tool versions; and each file’s normalized distribution name, exact version, filename, size, origin, and SHA-256. The bundle MUST contain the complete non-torch target closure plus exact installer/build tooling. No compatible wheel means stop unless a derived wheel is prepared from the lock-recorded sdist.
4. A derived-wheel record MUST retain the original sdist and its lock-matching SHA-256, every build-requirement file and SHA-256, builder OS/architecture/Python/ABI, build frontend/backend and versions, build command, and resulting wheel filename/size/SHA-256. Derived wheels are installed only after this record verifies; they do not amend dependency truth.
5. Environment construction MUST be fresh and network-disabled. Completion requires exact resolved-version checks, required imports, `pip check`, and an installed-environment manifest recording Python, platform, loaded modules, installer versions, CUDA/torch identity, all installed distributions, repository revision, lock/export hashes, and acquisition-manifest hash.
6. A10 schema v1 is unchanged. Every production certification-lane A10 using this environment MUST populate its existing `environment` fields and carry existing-format `inputs` references with roles `cluster_requirements`, `environment_acquisition_manifest`, and `environment_install_manifest`. R5-X2 additionally requires `transformer_lens_equivalence_report`. Each reference uses the SHA-256 of the exact referenced file bytes and an existing `local:` or `tamia:` location.
7. The committed lock/export remain in git. The acquisition bundle, original sdists, derived wheels, build inputs, manifests, and equivalence report MUST remain immutable and resolvable for the retention lifetime of every A10 and downstream artifact produced from them. They may not be deleted while referenced.
8. ED-33’s 3.2.1 branch is selected. An isolated 3.4.0 comparison environment is permitted solely as verification apparatus and is not a supported production profile; it may produce no A6 or downstream scientific artifact. It must differ from the production comparison environment only in `transformer-lens`. For `hm03l7yz`, it MUST demonstrate exact equality of tokenization, positions, and `blocks.28.hook_resid_post` activations over the complete configured R5-X2 evaluation token stream, and the 3.2.1 environment MUST pass the full-weight SAE forward sanity check. Failure or incomplete evidence leaves certification blocked and does not authorize an automatic move to 3.4.0.
...
Legacy backfill (ED-5): the ~~six-plus~~ **[ED-35: five evidenced — four + `hm03l7yz` recovered via R4-X1; see below]** pre-blueprint checkpoints receive backfilled A5 manifests — directory-hashed on the cluster per D1, `store_hash: null`, config and `tokens_trained` taken from their training configs and logs. This backfill is WP2's first task; retro-certificates subject the backfilled hashes.
Consumers: SS4, SS5, SS6, SS7 (via SS5 max units).

**ED-35 (authoritative WP2 checkpoint population, decided at WP2 population audit).** "Six-plus / all existing" is a **pre-blueprint estimate, superseded by assembled evidence** — same discipline as ED-33: a written number yields to what verification finds. The evidenced population of *distinct, eligible, existing final* checkpoints is **five** (four confirmed at ruling time + `hm03l7yz` recovered by authoritative Tamia access, R4-X1): `d1bgp5v5` (L16×32), `rwu04lpb` (L28×32), `zf2o13m2` (L40×32), `o1cx1dow` (L28×64) — each with A5 + A6 — and `hm03l7yz` (L28×16), recovered on-cluster, pending A5 backfill + A6 cert. The original "6+" over-counted (it swept in an intermediate, a retired poutine SAE, and telemetry IDs), but it was *right* that an L28×16 existed — the eligible population is five. Every other name gets a **recorded** disposition — none implicitly waived (ED-9 honest-state doctrine):
- **`a520ytu6`** → duplicate/intermediate of **`o1cx1dow`** (its 266,667,008-token resume source; the final is 400M). An intermediate training snapshot is not a distinct artifact (ED-27 identity logic) — no separate A5/A6.
- **`de575ae6` / `a0g2os3u` / `final_200003584`** (Montreal/poutine SAE) → **retired / out of scope**, authoritative reason: the poutine research direction was dropped. Out of scope by that decision *independent of* recoverability (there is also no checkpoint dir / A5 / A6 / location in the current repo). Not a WP2 gap.
- **`L28×16` = `hm03l7yz`** → **IN SCOPE — R4-X1 resolved by authoritative Tamia access; the checkpoint exists and completed.** `/scratch/y/yazid/qwen-sae-interp/results/sae_checkpoints/hm03l7yz/final_400001024` holds the full SAELens checkpoint (cfg.json, sae_weights.safetensors, trainer_state.pt, runner_cfg.json, …), preceded by 8 intermediate checkpoints — a run that reached `final_400001024`. `cfg.json` (sha256 `5dde4845…`) confirms `d_sae` 81920 (16×), `blocks.28.hook_resid_post`, TopK k=100, `sae_lens_version` 6.44.2 — **the same ED-33 6.x baseline as the other four**, so it slots into the existing cert lane with no new baseline question. The prior "unknown/unrecoverable" was a *session-access* limitation, never a fact about the artifact — which is exactly why ED-9 forbade inferring nonexistence from local absence, and why holding WP2 open was correct. Remaining work is the standard pipeline, not recovery: backfill A5 (ED-5/ED-27 identity = `hash_checkpoint_dir({cfg.json, sae_weights.safetensors})` computed on-cluster per D1 — NOT the raw per-file sha256s, which are provenance cross-checks only; `wandb: null` and `training_provenance.transformer_lens: null` are honest absences per ED-30, the W&B run being unrecoverable under the current entity), then A6 certify once the §6 gate opens. R4-X1 is closed.
- **W&B run IDs** → telemetry, not checkpoints (category error); out of scope.

Requirement status: the literal "6+" is **no longer an authoritative count-to-hit**; it is reinterpreted as "all existing **eligible** checkpoints," which the evidence resolves to **five** (R4-X1 recovered the L28×16). The frozen architecture's "all existing checkpoints" (`infrastructure_architecture.md` :131/:253) is **satisfied**. WP2 is complete over the eligible population once **all five** A6 certificates land (the four, plus `hm03l7yz` after its A5 backfill); it is **no longer partial-pending** — the one open contingency closed positive (artifact exists), so recovery is done and only the standard backfill+cert pipeline remains, gated as ever on §6. This **five-checkpoint** set is the authoritative basis for certification coverage and band-calibration provenance. Coverage (now complete): a clean grid centered on L28 — layer sweep 16/28/40 at 32×, **and** width sweep 16×/32×/64× at L28 (`hm03l7yz` / `rwu04lpb` / `o1cx1dow`). The 16× width point ED-35 first flagged as absent *did* exist; it is recovered, not a gap.
...
| `test_delta_golden` | **hard** | clamp (and add_direction) on tiny fixture reproduces pinned delta tensor within 128 ULP in fp32 (ED-33 / ED-26: measured max 32 ULP for clamp and 4 ULP for add_direction under the current 6.x TopK stack; golden bytes unchanged) |
...
| 2 | SS4 certify | legacy A5 backfill (ED-5), metrics job + bands file + report card; retro-run on all ~~6+~~ **five evidenced (ED-35: four + `hm03l7yz`)** checkpoints; **plus pulled-forward minimal SS10 subset**: `new_run_card`/`RunCardHandle.finalize` per the frozen interface (ED-6), and minimal `sync_registry` (§3.3, already fully specified) | Leaf | 0, 1 |
