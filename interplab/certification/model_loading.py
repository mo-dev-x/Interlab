...
activations certification measures. So `resolve_hf_model_snapshot` only
downloads the pinned revision to a local directory; every model, regardless
of ref scheme, is then constructed by the one loader below.
"""
...
def resolve_model_location(location: str) -> Path:
    ...
    through the pinned-download acquisition step above. The caller always
    ends up with a plain directory to hand to `load_local_hooked_transformer`
    (ED-34: one loader, never one per scheme)."""
    parsed = uris.parse(location)
