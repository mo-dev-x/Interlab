#!/bin/bash
#SBATCH --job-name=qwen-phase2-cheese
#SBATCH --output=slurm/logs/%j_phase2_cheese.out
#SBATCH --error=slurm/logs/%j_phase2_cheese.err
...
mkdir -p slurm/logs \
    results/feature_survey_l16_32x_wide \
    results/steering_sweep_instruct/cheese_l28_16x \
    results/steering_sweep_instruct/cheese_l40_32x

# Node 1: widen the l16_32x survey (top_n 150 -> 600) since no clean cheese
# feature turned up in the top 150 -- its cheese-context candidates all had
# garbled, non-cheese logit attributions there.
echo "Launching: survey l16_32x wide (top_n=600)"
srun --ntasks=1 --nodes=1 --exclusive \
    python scripts/survey_features.py \
        --sae_path results/sae_checkpoints/d1bgp5v5/final_400001024 \
        --model_name Qwen/Qwen2.5-14B-Instruct \
        --hook_layer 16 \
        --top_n 600 \
        --out_dir results/feature_survey_l16_32x_wide \
    > "slurm/logs/${SLURM_JOB_ID}_survey_l16_32x_wide.out" \
    2> "slurm/logs/${SLURM_JOB_ID}_survey_l16_32x_wide.err" &

# Node 2 + 3: steer the two already-confirmed clean cheese features across
# (layer, dict size) -- the Golden-Gate-Claude-style cross-comparison:
# (feature_id, output_name, random_control_id, scales, sae_path, hook_layer)
RUNS=(
